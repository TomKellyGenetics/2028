...
There are many different ways that twothousandandtwentyeight could implement the pipe. The goal of this document is to elucidate the variations, and the various pros and cons of each approach. This document is primarily aimed at the twothousandandtwentyeight developers (so we don't forget about important considerations), but will be of interest to anyone who wants to understand pipes better, or to create their own pipe that makes different tradeoffs
...
-   **Eager (mask)**, masking environment
    ...
    This is essentially how `%>%` has been implemented prior to twothousandandtwentyeight 2.0:
...
We have implemented three approaches in twothousandandtwentyeight:
...
```{r, eval = TRUE}
`%|>%` <- twothousandandtwentyeight::pipe_nested
```
...
#> 4: g(f(faulty()))
#> 3: h(g(f(faulty())))
#> 2: .External2(twothousandandtwentyeight_pipe) at pipe.R#181
#> 1: faulty() %|>% f() %|>% g() %|>% h()
```
...
```{r, eval = TRUE}
`%!>%` <- twothousandandtwentyeight::pipe_eager_lexical
```
...
#> 4: stop("tilt")
#> 3: faulty()
#> 2: .External2(twothousandandtwentyeight_pipe) at pipe.R#163
#> 1: faulty() %!>% f() %!>% g() %!>% h()
```
...
```{r, eval = TRUE}
`%?>%` <- twothousandandtwentyeight::pipe_lazy_masking
```
...
#> 4: g(.)
#> 3: h(.)
#> 2: .External2(twothousandandtwentyeight_pipe) at pipe.R#174
#> 1: faulty() %?>% f() %?>% g() %?>% h()
```
